...
conditionalValue = -.05
standardizedConditionalValue = (conditionalValue - means[2]) / stdDevs[2] 


l <- getMarginalDensity(copula_dist, 1, c(0,standardizedConditionalValue ) )
...
# plot(conditionalXValues , conditionalDist ,col='red',lwd=2)
...
stressFactor = getPercentChangesByYear('XLY', '2008-09-01')
stressFactor = stressFactor['::2009-03-01']
portfolioValue = matrix(1, nrow = 1000)
listOfPortfolioValues = c()

for (index in 1:length(stressFactor))
{
  
  conditionalValue = stressFactor[index]
  standardizedConditionalValue = (conditionalValue - means[2]) / stdDevs[2] 
  ...
  # conditionalVaR = inverseCDF(conditionalDist, conditionalXValues,  .95)
  # 
  # portfolioValue = portfolioValue * (1 + conditionalVaR)
  # cat(index, portfolioValue, '\n')
  # listOfPortfolioValues = c(listOfPortfolioValues, portfolioValue)
}
plot.ts(listOfPortfolioValues)
